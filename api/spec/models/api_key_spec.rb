...
RSpec.describe ApiKey, type: :model do
  ...
  describe "validations" do
    ...
    # key_digest and prefix are auto-generated by before_validation callback,
    # so shoulda matchers can't test presence directly
    it 'auto-generates key_digest before validation' do
      api_key = build(:api_key)
      api_key.valid?
      expect(api_key.key_digest).to be_present
    end

    it 'validates key_digest uniqueness' do
      existing = create(:api_key)
      duplicate = build(:api_key)
      duplicate.key_digest = existing.key_digest
      expect(duplicate).not_to be_valid
      expect(duplicate.errors[:key_digest]).to include('has already been taken')
    end

    it 'auto-generates prefix of length 8' do
      api_key = build(:api_key)
      api_key.valid?
  ...
  describe "callbacks" do
    ...
    it "display_key persists on same instance but is nil on fresh load" do
      api_key = create(:api_key)
      expect(api_key.display_key).to start_with("tp_")

      # Instance variables persist through reload
      api_key.reload
      expect(api_key.display_key).to start_with("tp_")

      # But a fresh load from DB won't have the raw key
      fresh_key = ApiKey.find(api_key.id)
      expect(fresh_key.display_key).to be_nil
    end
  end
